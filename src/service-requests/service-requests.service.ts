...
export interface AtolConsentReviewInput {
    consentId: number;
    city: string;
    clientName: string;
    inn: string;
...
export class ServiceRequestsService {
    ...
    async createAtolConsentReviewRequest(identity: ServiceRequestIdentity, consent: AtolConsentReviewInput) {
        const user = await this.usersService.getOrCreateOrUpdate(identity.chatId, identity.name, identity.username, identity.platform);
        await this.organizationsService.assertUserOrganization(identity.chatId, identity.platform, identity.organizationId);
        await this.ensureDefaultTypes();
        ...
        const request = await this.serviceRequestsRepo.save(this.serviceRequestsRepo.create({
            ...
            answers: {
                ...
                representativeBasis: consent.representativeBasis,
                generatedPdfPath: consent.generatedPdfPath,
                signedConsentPath: consent.signedFilePath,
                signedConsentName: consent.signedFileName,
            },
        }));

        await this.addEvent(request, 'created', 'client', 'Получено подписанное согласие на доступ АТОЛ', {
            consentId: consent.consentId,
            signedConsentName: consent.signedFileName,
        });
        await this.activityService.add({
            ...
            type: 'service_request_created',
            title: serviceType.title,
            description: `Создана сервисная заявка #${request.id} по подписанному согласию АТОЛ`,
            serviceRequestId: request.id,
