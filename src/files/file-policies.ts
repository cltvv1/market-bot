...
export const FILE_POLICIES: Record<FilePurpose, FilePolicy> = {
    'registration-photo': policy(12 * MB, ['image/jpeg', 'image/png', 'image/webp'], ['.jpg', '.jpeg', '.png', '.webp'], true, true),
    'ticket-image': policy(12 * MB, ['image/jpeg', 'image/png', 'image/webp', 'image/gif'], ['.jpg', '.jpeg', '.png', '.webp', '.gif'], true, true),
    'ticket-document': policy(20 * MB, ['application/pdf', 'text/plain', 'application/zip'], ['.pdf', '.txt', '.zip'], false, true),
    'ticket-audio': policy(30 * MB, ['audio/mpeg', 'audio/ogg', 'audio/mp4', 'audio/webm'], ['.mp3', '.ogg', '.m4a', '.webm'], true, true),
    'ticket-video': policy(80 * MB, ['video/mp4', 'video/webm', 'video/quicktime'], ['.mp4', '.webm', '.mov'], true, true),
    'service-invoice': policy(15 * MB, ['application/pdf'], ['.pdf'], false, false),
...
export function assertFilePolicy(
    ...
    if (buffer.length > policy.maxBytes) throw new BadRequestException('File exceeds the configured size limit');
    if (policy.serverGeneratedOnly && !serverGenerated) throw new BadRequestException('This file category is server-generated only');
    const detected = detectMime(buffer);
    const mime = detected ?? suppliedMime?.toLowerCase();
    if (!mime || !policy.mimeTypes.includes(mime)) throw new BadRequestException('File content type is not allowed');
