...
async function main() {
    ...
    try {
        ...
        const historicalOrgName = 'x'.repeat(1500);
        await db.manager.update(RegistrationRequestEntity, id, { orgName: historicalOrgName });
        await customer.reload();
        await field(customer, 'orgName').waitFor();
        assert.equal(await field(customer, 'orgName').inputValue(), historicalOrgName);
        assert.equal(await field(customer, 'orgName').getAttribute('maxlength'), '10000');
        await field(customer, 'kktModel').fill('Updated alongside historical text');
        await customer.getByRole('button', { name: 'Сохранить черновик', exact: true }).click();
        await customer.getByText('Черновик сохранён', { exact: true }).waitFor();
        await customer.reload();
        await field(customer, 'orgName').waitFor();
        assert.equal(await field(customer, 'orgName').inputValue(), historicalOrgName);
        assert.equal(await field(customer, 'kktModel').inputValue(), 'Updated alongside historical text');
        check('existing 1500-character value resumes and saves unchanged when editing another field');
        await field(customer, 'orgName').fill('Демо-мастерская «Контур»');
        await customer.getByRole('button', { name: 'Сохранить черновик', exact: true }).click();
        await customer.getByText('Черновик сохранён', { exact: true }).waitFor();
