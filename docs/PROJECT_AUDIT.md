...
- импорта/экспорта 1С.
- В интеграционной ветке добавлен shadow-only фундамент АТОЛ Connect/Платформы ОФД: mappings, runs/errors, exclusions, equipment sync, observations и service opportunities. До слияния и эксплуатационной сверки он не считается production-интеграцией.
...
- сервисная форма по умолчанию использует реальный API и защищённую web-сессию;
- каталог сервисной формы загружается из общих backend `service_types`;
- сайт создаёт `fn_replacement`, `firmware_update` и `kkt_remote_work` через тот же workflow, что Telegram и MAX;
- реальная заявка получает последовательный `SR-{id}`, отображается в админке и доступна на странице статуса в той же web-сессии;
- ошибки отправки показываются пользователю.

Подтверждено частично:

- дополнительные поля простой заявки сохраняются в общем `problemDescription`, а телефон — в `contactForCall`;
- общие вложения сервисной заявки пока не реализованы и поэтому не предлагаются формой;
- публичная проверка статуса привязана к cookie-сессии браузера, отдельного токена для переноса между устройствами пока нет.
