...
| --- | --- | --- | --- | --- |
| Web payment proof: BACKEND_AVAILABLE / UI_NOT_CONNECTED (P-PROOF branch) | Dedicated owner-session POST/GET C/service-requests/:id/payment-proof; `ServiceRequestPaymentProofService` creates the canonical pointer with required expectedVersion | Generic reply upload remains kind `message`, never proof; public-token upload intentionally absent | Backend implemented in this branch, FE-1C not implemented | FE-1C must connect owner workflow and document projection; no payment truth from upload |
| Bot payment proof: AVAILABLE (P-PROOF branch hardening) | Telegram/MAX capture exact request/version before download and call the same `ServiceRequestPaymentProofService`; strict pending-to-active binding | Current proof display preserves staff RBAC; owner download requires full canonical binding | No | Shared ServiceRequest command, no OrderDocument model or profile merging |
