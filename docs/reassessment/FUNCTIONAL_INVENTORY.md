...
Регистрация ККТ теперь имеет статус `CODE_CONFIRMED`: web, Telegram и MAX
создают прежний `RegistrationRequest`, но используют единый checklist ККТ/ФН/ОФД.
Фото не обязательно для первичной отправки; поздние значения и файлы доступны
владельцу. Оператор проверяет комплектность, а backend блокирует final PDF и
передачу инженеру до readiness. Ручной browser smoke фиксируется отдельно в PR.
