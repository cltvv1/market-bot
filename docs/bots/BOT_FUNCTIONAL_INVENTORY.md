...
| 11 | Cancel ATOL draft | callback | yes | yes | request deletion | managed-file cleanup is incomplete |
...
| 14 | Customer attachment | media | yes | partial | ticket media | TG types are broader; remote references may expire |
| 15 | Operator connects | callback | yes | yes | users/tickets | relationship persists, forwarding mode does not |
| 16 | Operator text chat | text | yes | yes | direct send and ticket message | transcript persists; restart breaks routing |
| 17 | Operator media | attachment | yes | partial | ticket metadata | TG copies media; MAX sends text/URL |
...
| 21 | OFD activation | service button | exposed only | no | none | Telegram callback has no handler |
| 22 | Legacy in-bot admin actions | admin callbacks | yes | no | direct registration/ticket services | no role check in handlers |
...
- Stale callback handling varies. Client service-request ownership is checked;
  legacy Telegram admin callbacks use numeric IDs directly.
...
`src/service-requests/service-request.flows.spec.ts`. No direct Telegram or MAX
update-handler suite was found.
