...
function pdf(label = 'Synthetic fixture') {
    const text = String(label).replace(/[^\x20-\x7e]/g, '_').replace(/[()\\]/g, '_');
    const stream = `BT /F1 12 Tf 10 10 Td (${text}) Tj ET`;
    const objects = [
...
function fixture(name) {
    return fs.readFileSync(path.join(__dirname, 'files', name));
}

function zip() {
    const name = Buffer.from('note.txt');
    const content = Buffer.from('Synthetic ZIP fixture\n');
...
module.exports = { pdf, fixture, zip };
