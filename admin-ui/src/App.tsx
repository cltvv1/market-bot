...
import type {
    ...
    OpportunityStatus, Priority, Registration, ServiceAttachment, ServiceEvent,
...
function ServiceRequests( props: ListProps & { responsible: string; permissions: string[] }, ) { const statuses = serviceApiStatuses(props.status); const paths = statuses.map( (status) => `/admin/api/service-requests?${new URLSearchParams({ status, ...(props.platform ? { platform: props.platform } : {}) })}`, ); const [items, setItems] = useState<ServiceRequest[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [selectedId, setSelectedId] = useState<number>(); const [manualOpen, setManualOpen] = useState(false); const [manualSource, setManualSource] = useState<'phone' | 'admin'>( 'phone', ); const [manualType, setManualType] = useState('kkt_remote_work'); const [manualName, setManualName] = useState(''); const [manualPhone, setManualPhone] = useState(''); const [manualDescription, setManualDescription] = useState(''); useEffect(() => { setLoading(true); Promise.all(paths.map((path) => api<ServiceRequest[]>(path))) .then((groups) => setItems(groups.flat())) .catch(() => setError('Ошибка загрузки')) .finally(() => setLoading(false)); }, [paths.join('|'), props.refreshKey]); const filtered = items.filter( (item) => (!props.priority || (item.priority || 'normal') === props.priority) && (!props.responsible || `${item.responsibleOperatorId || ''} ${item.executorName || ''}` .toLowerCase() .includes(props.responsible.toLowerCase())), ); useEffect(() => { const id = props.requestedId || filtered[0]?.id; if (id) setSelectedId(id); }, [items, props.requestedId, props.priority, props.responsible]); const createManual = async () => { if ( !manualName.trim() || !manualPhone.trim() || !manualDescription.trim() ) return; const created = await post<{ request: ServiceRequest }>( '/admin/api/service-requests/manual', { source: manualSource, serviceTypeCode: manualType, initialStatus: 'review_required', contactSnapshot: { name: manualName, phone: manualPhone, preferredChannel: 'phone', }, answers: { contactName: manualName, phone: manualPhone, description: manualDescription, }, }, ); setManualOpen(false); setManualName(''); setManualPhone(''); setManualDescription(''); setSelectedId(created.request.id); props.onChanged(); }; return ( <> {props.permissions.includes('serviceRequests.update') && ( <div className="manual-request-tools"> <button onClick={() => setManualOpen((value) => !value)}> <UserPlus size={16} /> Создать вручную </button> {manualOpen && ( <div className="operator-panel manual-request-form"> <div className="form-row"> <select value={manualSource} onChange={(event) => setManualSource( event.target.value as | 'phone' | 'admin', ) } > <option value="phone"> Обращение по телефону </option> <option value="admin"> Создано администратором </option> </select> <select value={manualType} onChange={(event) => setManualType(event.target.value) } > <option value="kkt_remote_work"> Удалённая работа с ККТ </option> <option value="firmware_update"> Обновление прошивки </option> <option value="fn_replacement"> Замена ФН </option> </select> </div> <div className="form-row"> <input value={manualName} onChange={(event) => setManualName(event.target.value) } placeholder="Контактное лицо" /> <input value={manualPhone} onChange={(event) => setManualPhone(event.target.value) } placeholder="Телефон" /> </div> <textarea value={manualDescription} onChange={(event) => setManualDescription(event.target.value) } placeholder="Описание обращения" /> <button className="primary" onClick={() => void createManual()} > <Check size={16} /> Создать заявку </button> </div> )} </div> )} <Workbench title="Заявки по сервису" items={filtered} selectedId={selectedId} onSelect={setSelectedId} loading={loading} error={error} row={(item) => ( <> <div className="row-top"> <strong> {item.requestNumber || `Заявка #${item.id}`} ·{' '} {item.serviceTypeTitle} </strong> <span>{fmtDate(item.createdAt)}</span> </div> <div className="badges"> <Badge>{statusText(item.status)}</Badge> <Badge priority={item.priority}> {priorityText(item.priority)} </Badge> </div> <span className="row-preview"> {item.executorName ? `Исполнитель: ${item.executorName}` : item.source || item.platform} </span> </> )} detail={ selectedId ? ( <ServiceDetail id={selectedId} onNavigate={props.onNavigate} onChanged={props.onChanged} permissions={props.permissions} /> ) : null } /> </> ); }

function ServiceDetail({ id, onNavigate, onChanged, permissions, }: { id: number; onNavigate: Navigate; onChanged: () => void; permissions: string[]; }) { type Detail = { request: ServiceRequest; events: ServiceEvent[]; messages: ServiceMessage[]; attachments: ServiceAttachment[]; }; const [data, setData] = useState<Detail>(); const [card, setCard] = useState<CustomerCard | null>(null); const [priority, setPriority] = useState<Priority>('normal'); const [comment, setComment] = useState(''); const [invoice, setInvoice] = useState<File>(); const [address, setAddress] = useState(''); const [visitTime, setVisitTime] = useState(''); const [engineers, setEngineers] = useState<Staff[]>([]); const [engineerId, setEngineerId] = useState(''); const [message, setMessage] = useState(''); const [messageVisibility, setMessageVisibility] = useState< 'customer' | 'internal' >('customer'); const allowed = (permission: string) => permissions.includes(permission) && (permission !== 'serviceRequests.payment' || (data?.request.status === 'waiting_payment' && Boolean(data.request.paymentProofFileId))); const load = useCallback( () => api<Detail>(`/admin/api/service-requests/${id}`).then((result) => { setData(result); setPriority(result.request.priority || 'normal'); setComment(result.request.operatorComment || ''); setEngineerId( result.request.assignedEngineerId ? String(result.request.assignedEngineerId) : '', ); }), [id], ); useEffect(() => { setCard(null); void load(); }, [load]); useEffect(() => { if (allowed('serviceRequests.assign')) void api<Staff[]>('/admin/api/staff/engineers').then(setEngineers); }, [permissions.join('|')]); useEffect(() => { if ( data?.request.status !== 'waiting_payment' || data.request.paymentProofFileId ) return; const timer = window.setInterval(() => { void load(); }, 5000); return () => window.clearInterval(timer); }, [load, data?.request.status, data?.request.paymentProofFileId]); if (!data) return <Empty text="Загрузка заявки..." />; const request = data.request; const answers = Object.entries(request.answers || {}).filter( ([key]) => !['generatedPdfPath', 'signedConsentPath'].includes(key), ); if (request.paymentProofFileId) answers.unshift([ 'paymentProof', `/admin/api/service-requests/${id}/payment-proof`, ]); const save = async () => { await post(`/admin/api/service-requests/${id}/operator-state`, { priority, operatorComment: comment, }); await load(); onChanged(); }; const assign = async () => { if (!engineerId) return; await post(`/admin/api/service-requests/${id}/assign-engineer`, { assignedEngineerId: Number(engineerId), }); await load(); onChanged(); }; const uploadInvoice = async () => { if (!invoice) return; const form = new FormData(); form.append('file', invoice); await upload(`/admin/api/service-requests/${id}/invoice-file`, form); await load(); }; const action = async (name: string) => { await post(`/admin/api/service-requests/${id}/${name}`); await load(); onChanged(); }; const schedule = async () => { if (!address) return; await post(`/admin/api/service-requests/${id}/schedule`, { visitAddress: address, visitTime, operatorComment: comment, }); await load(); onChanged(); }; const sendMessage = async () => { if (!message.trim()) return; await post(`/admin/api/service-requests/${id}/messages`, { text: message, visibility: messageVisibility, }); setMessage(''); await load(); }; const openCard = async () => setCard(await loadCustomerCard(request)); if (card) return ( <CustomerCardView card={card} onClose={() => setCard(null)} onNavigate={onNavigate} /> ); return ( <> <DetailHeader title={`${request.requestNumber || `Заявка #${id}`} · ${request.serviceTypeTitle}`} subtitle={`${request.source || request.platform} · ${statusText(request.status)} · ${fmtDate(request.createdAt)}`} onCustomer={ allowed('organizations.read') ? openCard : undefined } /> <div className="detail-body"> <FieldGrid fields={[ ['Статус', statusText(request.status)], ['Приоритет', priorityText(request.priority)], ['Клиент', request.contactSnapshot?.name], ['Телефон', request.contactSnapshot?.phone], ['Куратор', request.responsibleOperatorId], ['Исполнитель', request.executorName], ['Внутренний комментарий', request.operatorComment], [ 'Стоимость', request.calculatedPrice ? `${request.calculatedPrice} ₽` : 'Не рассчитана', ], ...answers.map( ([key, val]) => [answerLabels[key] || key, val] as [ string, unknown, ], ), ]} /> {data.attachments.length > 0 && ( <div className="service-attachments"> <h3>Файлы заявки</h3> {data.attachments.map((attachment) => ( <a className="button" key={attachment.id} href={`/admin/api/service-requests/${id}/attachments/${attachment.id}`} target="_blank" rel="noreferrer" > <FileText size={16} /> {attachment.file.originalName || `Файл ${attachment.id}`} </a> ))} </div> )} <div className="messages event-list"> {data.events?.map((event) => ( <div className="message" key={`event-${event.id}`}> <div> {event.type === 'answered' && event.payload?.value !== undefined ? String(event.payload.value) : event.message || event.type} </div> <span> {event.actor} · {fmtDate(event.createdAt)} </span> </div> ))} {data.messages?.map((item) => ( <div className={`message ${item.visibility === 'internal' ? 'message--internal' : ''}`} key={`message-${item.id}`} > <div>{item.text}</div> <span> {item.authorType} ·{' '} {item.visibility === 'internal' ? 'внутреннее' : 'видит клиент'}{' '} · {fmtDate(item.createdAt)} </span> </div> ))} </div> {allowed('serviceRequests.update') && ( <div className="operator-panel"> <div className="form-row"> <PrioritySelect value={priority} onChange={setPriority} /> <button className="primary" onClick={save}> <Check size={16} /> Сохранить </button> </div> <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Внутренний комментарий или поручение" /> <div className="form-row"> <select value={messageVisibility} onChange={(event) => setMessageVisibility( event.target.value as | 'customer' | 'internal', ) } > <option value="customer"> Сообщение клиенту </option> <option value="internal"> Внутренняя заметка </option> </select> <input value={message} onChange={(event) => setMessage(event.target.value) } placeholder={ messageVisibility === 'customer' ? 'Ответ клиенту' : 'Заметка для сотрудников' } /> <button onClick={() => void sendMessage()}> <Send size={16} /> Отправить </button> </div> {allowed('serviceRequests.assign') && ( <div className="form-row"> <select value={engineerId} onChange={(e) => setEngineerId(e.target.value) } > <option value="">Выберите инженера</option> {engineers.map((engineer) => ( <option value={engineer.id} key={engineer.id} > {engineer.displayName} </option> ))} </select> <button onClick={assign}>Назначить</button> </div> )} {allowed('serviceRequests.invoice') && ( <div className="form-row"> <input type="file" accept="application/pdf" onChange={(e) => setInvoice(e.target.files?.[0]) } /> <button onClick={uploadInvoice}> Загрузить PDF </button> {request.invoiceFileId && ( <a className="button" href={`/admin/api/service-requests/${id}/invoice`} target="_blank" rel="noreferrer" > Скачать счёт </a> )} {Boolean( request.answers?.signedConsentPath, ) && ( <a className="button" href={`/admin/api/service-requests/${id}/signed-consent`} target="_blank" rel="noreferrer" > Согласие </a> )} </div> )} {allowed('serviceRequests.schedule') && ( <div className="form-row"> <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Адрес визита" /> <input type="datetime-local" value={visitTime} onChange={(e) => setVisitTime(e.target.value) } /> <button onClick={schedule}> Назначить визит </button> </div> )} <div className="actions"> {allowed('serviceRequests.payment') && ( <button onClick={() => action('payment-received')} > Оплата получена </button> )} {allowed('serviceRequests.close') && ( <> <button onClick={() => action('complete')}> Завершить </button> <button className="danger" onClick={() => action('cancel')} > Отменить </button> </> )} </div> </div> )} </div> </> ); } // prettier-ignore
...
function serviceApiStatuses(status: string) { if (status === 'all') return ['all']; if (status === 'closed') return ['completed', 'closed', 'cancelled']; if (status === 'waiting_payment') return ['waiting_payment']; if (status === 'in_work') return ['draft', 'submitted', 'price_confirmed', 'review_required', 'clarification_required', 'invoice_required', 'paid', 'scheduled', 'in_progress']; return ['active']; }
