...
function RegistrationDetail({
    ...
    const [status, setStatus] = useState(
        summary.isProcessed ? 'processed' : summary.status || 'new',
    );
...
function ServiceRequests(
    ...
    const filtered = items.filter(
        (item) =>
            ...
            (!props.responsible ||
                `${item.responsibleOperatorId || ''} ${item.executorName || ''}`
                    .toLowerCase()
        ...
        >
            {message.messageType === 'image' && url ? (
                <a href={url} target="_blank">
                    <img src={url} alt={message.fileName || 'Изображение'} />
                </a>
            ) : message.messageType && message.messageType !== 'text' && url ? (
                <a href={url} target="_blank">
                    {message.fileName || message.text || 'Открыть файл'}
                </a>
            ) : (
